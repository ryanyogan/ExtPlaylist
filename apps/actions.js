...
const _searchVideos = (dispatch, keyword, nextPageToken = null) => {
  ...
  let url = `${youtubeApiBaseUrl}/search?part=snippet&q=${encodedKeyword}
    &type=video&maxResults=10&key=${youtubeApiKey}`;
  if (nextPageToken) {
  ...
  return fetch(url)
    ...
    .then((data) => {
      if (nextPageToken) {
        dispatch(_moreSearchResultRecieved(data));
      } else {
...
const _searchStarted = (keyword) => ({ type: SEARCH_STARTED, keywrd });
const _searchResultReceived = (data) => ({ type: SEARCH_RESULT, data });
const _moreSearchResultRecieved = (data) => ({ type: MORE_SEARCH_RESULT, data });
